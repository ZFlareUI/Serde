...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!("\n Recording supplier performance...");
    
    let order_value = Money::new(dec!(1000.00), Currency::usd());
    
    supplier.performance.update_with_order(order_value.clone(), true, true);  // On time, good quality
    supplier.performance.update_with_order(order_value.clone(), false, true); // Late, good quality  
    supplier.performance.update_with_order(order_value, true, false); // On time, poor quality
    ...
    // Note: Order created in draft status - would need line items before submission
    ...
    let adjustment_transaction = StockTransactionBuilder::new()
        .product_id(product.id)
        .source_location(warehouse.id)
        .transaction_type(TransactionType::Adjustment)
        .quantity(dec!(-10))
        .unit_of_measure("units".to_string())
        .reason_code("Cycle Count Adjustment".to_string())
        .build()?;
        
    println!(" Created adjustment transaction: {} units", adjustment_transaction.quantity);
    ...
    let transaction_json = serde_json::to_string_pretty(&receipt_transaction)?;
    println!(" Serialized transaction to JSON ({} bytes)", transaction_json.len());
    ...
    println!(" Orders: Created with approval workflow");
    println!(" Transactions: Receipt and adjustment types");
    println!(" Serialization: JSON format working");
    println!("\nNext steps:");
    println!(" Complete repository implementations (SQL, MongoDB)");
    println!(" Build service layer with business logic");
    println!(" Add REST API with Axum framework");
    println!(" Implement configuration management");
    println!(" Add advanced analytics and forecasting");
